...
document.querySelectorAll('.start-btn').forEach((button, index) => {
  button.addEventListener('click', () => {
    ...
    // Ensure the response entry exists
    participantData.responses[chartIndex] = participantData.responses[chartIndex] || { chart: chartIndex + 1 };

    if (stepType === 0) {
      const trend = document.querySelector(`input[name="trend${chartIndex}"]:checked`);
      participantData.responses[chartIndex].trend = trend ? trend.value : null;
    } else if (stepType === 1) {
      const confidence = document.querySelector(`input[name="confidence${chartIndex}"]:checked`);
      participantData.responses[chartIndex].confidence = confidence ? confidence.value : null;
    } else if (stepType === 2) {
